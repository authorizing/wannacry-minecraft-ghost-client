...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, float* p_scroll_v, float size_avail_v, float size_contents_v, ImDrawCornerFlags rounding_corners)
{
	...
	auto it_active = active_anim.find(id);

	if (it_active == active_anim.end())
	{
		active_anim.insert({ id, 0.f });
		it_active = active_anim.find(id);
	}
	if (hovered || IsItemActive())
	{
		if (it_active->second < style.ScrollbarSize / 2)
			it_active->second += style.ScrollbarSize / 10;
	}
	else
	{
		if (it_active->second > 0.f)
			it_active->second -= style.ScrollbarSize / 10;
	}

	GetOverlayDrawList()->AddRectFilled(ImVec2(grab_rect.Min.x - it_active->second, grab_rect.Min.y - it_active->second), grab_rect.Max + ImVec2(it_active->second, it_active->second), GetColorU32(ImGuiCol_ScrollbarGrabActive), style.ScrollbarRounding);
	window->DrawList->AddRectFilled(grab_rect.Min, grab_rect.Max, GetColorU32(ImGuiCol_ScrollbarGrab), style.ScrollbarRounding);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
	...
	const ImRect frame_bb(window->DC.CursorPos - ImVec2(0, 8), window->DC.CursorPos + frame_size - ImVec2(0, 8));
	...
	{
		RenderNavHighlight(frame_bb, id);
		RenderFrame(frame_bb.Min - ImVec2(2, 0), frame_bb.Max, ImColor(), true, 5);
	}
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
	...
	ImVec4 text_active = ImVec4(33 / 255.f, 150 / 255.f, 243 / 255.f, 1.f);
	ImVec4 text_inactive = ImVec4(242 / 255.f, 243 / 255.f, 249 / 255.f, 1.f); 

	static std::map<ImGuiID, ImVec4> text_anim;
	auto it_text = text_anim.find(id);

	if (it_text == text_anim.end()) {
		text_anim.insert({ id, text_inactive });
	...
	if (selected) {
		if (it_text->second.x > text_active.x)
			it_text->second.x -= 0.01f;
		if (it_text->second.y > text_active.y)
			it_text->second.y -= 0.01f;
		if (it_text->second.z > text_active.z)
			it_text->second.z -= 0.01f;
	...
	ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(it_text->second.x, it_text->second.y, it_text->second.z, 1.f));
	RenderText(ImVec2(pos.x + 5, pos.y), label);
	ImGui::PopStyleColor();
