...
void aim::handler() {
	...
	for (; "aim"; Sleep(1)) {
		...
		if (!wcry::minecraft->address || 
			!wcry::current_world->address || 
			!wcry::local_player->address || 
			wcry::minecraft->is_in_menu()) 
		continue;

		size_t selectet_player = 0; double closest_yaw = 500.0;
		
		vec3 my_pos = wcry::local_player->get_pos(); 

		float_t my_yaw = wcry::local_player->get_yaw(), my_pyaw = wcry::local_player->get_previous_yaw(),
		my_pitch = wcry::local_player->get_pitch(), my_ppitch = wcry::local_player->get_previous_pitch();

		for (size_t current : wcry::current_world->get_player_list()) {
		...
		if (closest_yaw != 500.0 && selectet_player) {
			...
			closest_crn.push_back(distance_c_4), wcry::_vec_sort(closest_crn);
			...
			if (closest_crnr_1.x == closest_crnr_2.x) { closest_point.z = my_pos.z; 
				...
				if (!wcry::_in_range(closest_point.z, closest_crnr_1.z, closest_crnr_2.z)) closest_point = closest_crnr_1;
			...
			if (closest_crnr_2.z == closest_crnr_2.z) { closest_point.x = my_pos.x;
				...
				if (!wcry::_in_range(closest_point.x, closest_crnr_1.x, closest_crnr_2.x)) closest_point = closest_crnr_1;
			...
			float tmp = wcry::mc_msense * 0.600f + 0.200f, actual_mc_sense = pow(tmp, 3) * 8.00f;
			
			if (abs(dif) < 2.0f) dif = wcry::_rand_val(1.f, 2.f) * (abs(dif) / dif);
			dif *= actual_mc_sense * (aim_speed + (int)wcry::_rand_val(1.0f, 4.0f));

			float new_yaw_a = my_yaw + (int)dif * 0.0015f, prv_yaw_a = my_pyaw + (int)dif * 0.0015f;
			wcry::local_player->set_yaw(new_yaw_a), wcry::local_player->set_previous_yaw(prv_yaw_a);
